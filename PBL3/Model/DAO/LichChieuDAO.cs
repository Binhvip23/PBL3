    ...
    internal class LichChieuDAO
    {
        ...
        public void ThemPhimDangChieu(int idLC, int idPhong)
        {
            using (var connection = new SqlConnection(ConnectionString))
            {
                ...
                SqlCommand command = new SqlCommand(query, connection);
                command.Parameters.AddWithValue("@idLC", idLC);
                command.Parameters.AddWithValue("idPhong", idPhong);
                command.ExecuteNonQuery();
            }
        }
        public void XoaPhimDangChieu(int idLC,int idPhong)
        {
            using(var connection = new SqlConnection(ConnectionString))
            {
                connection.Open();
                string query = @"DELETE FROM LichChieu_PhongChieu WHERE IdLichChieu = @idLC AND IdPhongChieu = @idPhong";
                SqlCommand command = new SqlCommand(query, connection);
                command.Parameters.AddWithValue("@idLC", idLC);
                command.Parameters.AddWithValue("@idPhong", idPhong);
                command.ExecuteNonQuery();
